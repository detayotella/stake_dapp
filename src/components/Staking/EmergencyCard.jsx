...
function EmergencyCard() {
  ...
  return (
    <div className="bg-[var(--card-background)] rounded-lg border border-[var(--border-color)] p-6 text-center">
      ...
      <div>
        <p className="bg-red-200 rounded-2xl"> Please Note as there is 20% penalty fee for every emergency withdrawal</p>
      </div>
      <button onClick={handleEmergencyWithdraw} className="mt-10 btn-secondary border-red-500 text-red-500 hover:bg-red-50 w-full h-12 rounded-lg font-semibold text-base">
        Emergency Withdraw
      </button>
    </div>
  );
